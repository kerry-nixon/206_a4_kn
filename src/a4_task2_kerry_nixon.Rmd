...
```{r}
ggplot(data = hares_juveniles, aes(x = hindft, y = weight)) +
  geom_point( size = 2) +
  theme_minimal() +
  labs(x = "\nFlipper length (mm)",
       y = "Body mass (g)\n")
...
hares_lm <- lm(weight ~ hindft, data = hares_juveniles)
...
hare_cor <- cor.test(hares_juveniles$weight, hares_juveniles$hindft)
